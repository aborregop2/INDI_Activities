...
{
  glm::mat4 TG(1.f);
  TG = glm::translate(TG, glm::vec3(0,21.f,ghostPos));
...
void MyGLWidget::projectTransform ()
{
  ...
  else Proj = glm::perspective(glm::radians(60.f), ra, 0.1f, 25.f);
...
void MyGLWidget::viewTransform ()
{
  if (!camera2) {
      ...
      glm::vec3 obs = glm::vec3(-5,25,0);
      glm::vec3 vrp = glm::vec3(0,23,0);
...
void MyGLWidget::carregaShaders() 
{
  ...
  LLoc = glGetAttribLocation (program->programId(), "L");
...
void MyGLWidget::c(bool b) {
  emit cam2(b);
}

void MyGLWidget::a() {
  emit changeMoveSig(false);
}

void MyGLWidget::changeMove() {
  makeCurrent();
  if (girant) atura();
  else engega();
  update();
